...
- **Agent Overview pane**: Right-click a terminal pane running Claude Code → **Show Agent Overview** to open a native (non-webview) reading pane pinned beside it. It shows what you last asked, a live activity strip of the agent's tool calls (`Bash zig build`, `Read capi.zig`, …) with a spinner while a call is still in flight, and the agent's last message rendered for reading: prose as prose, fenced code in its own monospaced block that scrolls horizontally so a long line never widens the pane. A **B** button in the header toggles bionic reading, which bolds the leading fraction of each word (prose only — code keeps its exact glyphs); the setting persists across restarts. The view is constrained to sit immediately beside its agent's pane and cannot be detached to another window or serialized into a session — it is reopened from the pane's menu. Built for Claude Code today (reads `~/.claude/projects/<cwd>/*.jsonl`), with the Claude-specific parsing isolated behind a single reader so other agents can be added later.
