...
struct AgentTranscriptTests {
    ...
    @Test func retainsTurnsForHeaderNavigation() {
        ...
        let turns = AgentTranscriptReader.parseTurns(lines: lines)

        #expect(turns.map(\.lastUserPrompt) == ["first", "second"])
    ...
    @Test func toolResultsDoNotCreateHistoryTurns() {
        let turns = AgentTranscriptReader.parseTurns(lines: [
            userLine("go"),
        ...
        #expect(turns[0].lastUserPrompt == "go")
...
struct CodexTranscriptTests {
    ...
    @Test func retainsCodexTurnsForHeaderNavigation() {
        let turns = CodexTranscriptReader.parseTurns(lines: [
            userLine("first"),
