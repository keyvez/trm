        ...
        if (poll_fds.items[1].revents & inp_flags != 0) {
            ...
            if (n_opt) |n| {
                if (n == 0) {
                    ...
                    // trm's in-tree ghostty-vt returns an error union here
                    // (upstream zmx's pinned ghostty did not); a state-tracking
                    // failure must not kill the daemon.
                    vt_stream.nextSlice(buf[0..n]) catch |err| {
                        std.log.warn("vt stream error err={}", .{err});
                    };
